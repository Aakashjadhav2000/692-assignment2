...
void configure_traffic_light(const char *led_name, const char *red_path, const char *yellow_path, const char *green_path, const char *state, uint32_t duration) {
    set_gpio_value(red_path, state[0] == '1' ? "1" : "0");
    set_gpio_value(yellow_path, state[1] == '1' ? "1" : "0");
    set_gpio_value(green_path, state[2] == '1' ? "1" : "0");
    printf("%s - Red: %s, Yellow: %s, Green: %s, Duration: %u seconds\n", led_name, state[0] == '1' ? "ON" : "OFF", state[1] == '1' ? "ON" : "OFF", state[2] == '1' ? "ON" : "OFF", duration);
}
...
int main(void) {
    ...
    while (1) {
        ...
        configure_traffic_light("LED1", LED1_RED_PATH, LED1_YELLOW_PATH, LED1_GREEN_PATH, "001", (uint32_t)(green_sig_time * 60));
        ...
        configure_traffic_light("LED1", LED1_RED_PATH, LED1_YELLOW_PATH, LED1_GREEN_PATH, "010", yellow_sig_time);
        ...
        configure_traffic_light("LED2", LED2_RED_PATH, LED2_YELLOW_PATH, LED2_GREEN_PATH, "001", (uint32_t)(green_sig_time * 60));

        (void)sleep((uint32_t)(green_sig_time * 60)); /* Wait for green signal time */

        /* Set the second traffic light to yellow */
        configure_traffic_light("LED2", LED2_RED_PATH, LED2_YELLOW_PATH, LED2_GREEN_PATH, "010", yellow_sig_time);
